...
def run_remote_tournament(name_1, name_2, log_dir, steps_per_game=45000, n_episodes=10, n_cores=-1, seed=None,
    ...
    Args:
        name_1: Name of the first docker image. Already is appended to the prefix
            "swr.eu-west-101.myhuaweicloud.eu/his_air_hockey_competition_eu/solution-"
        name_2: Name of the second docker image. Already is appended to the prefix
            "swr.eu-west-101.myhuaweicloud.eu/his_air_hockey_competition_eu/solution-"
        log_dir (str): The path to the log directory.
    ...
    def agent_builder(mdp, i, docker_name_1, docker_name_2):
        ...
        container_id_1 = client.api.create_container(BASE_NAME_IMG + docker_name_1, detach=True,
                                                     ...
                                                     command=f"python 2023-challenge/run.py -e tournament_server --example baseline --host 0.0.0.0 --port {port_1}")
        ...
        container_id_2 = client.api.create_container(BASE_NAME_IMG + docker_name_2, detach=True,
                                                     ...
                                                     command=f"python 2023-challenge/run.py -e tournament_server --example baseline --host 0.0.0.0 --port {port_2}")
        ...
        def log_docker_out():
            while True:
                try:
                    container_1.reload()
                    with open(os.path.join(log_dir, "out_1.log"), 'wb') as f:
                        for line in container_1.logs(stream=True, follow=False):
                            f.write(line)

                    container_2.reload()
                    with open(os.path.join(log_dir, "out_2.log"), 'wb') as f:
        ...
        thread = Thread(target=log_docker_out, daemon=True)
        thread.start()

        return RemoteTournamentAgentWrapper(host_1, port_1, host_2, port_2, container_id_1, container_id_2)

    # TODO get interpolation order setting from docker containers
    start = time.time()

    _run_tournament(log_dir, agent_builder, name_1, name_2, steps_per_game, n_episodes, n_cores, seed, generate_score,
                    quiet, render, save_away_data, docker_name_1=name_1, docker_name_2=name_2)
...
def _run_tournament(log_dir, agent_builder, name_1, name_2, n_steps=45000, n_episodes=10, n_cores=-1, seed=None,
    ...
    result_1 = {}
    winner = [game[1] for game in data]
    counter = Counter(winner)

    result_1["Overall Score"] = {name_1: counter[1], name_2: counter[2]}
    result_1["Mean Penalty Points"] = np.mean([game[2] for game in data])

    violations_1 = defaultdict(list)

    for game in data:
        for key, value in game[4]["Violation Summary"].items():
    ...
    if save_away_data:
        ...
        result_2["Overall Score"] = {name_1: counter[1], name_2: counter[2]}
        result_2["Mean Penalty Points"] = np.mean([game[3] for game in data])
...
def _run_single_tournament(log_dir, agent_builder, name_1, name_2, n_steps, quiet, render, seed, i, save_away_data,
    ...
    dataset, score, constraints_dict_1, constraints_dict_2, jerk_1, jerk_2, computation_time_1, computation_time_2, \
    ...
    return score, winner, penalty_sum_1, penalty_sum_2, violations_1, violations_2
...
def compute_metrics(core, eval_params):
    ...
    penalty_sum_1, violations_1 = get_violations(n_steps, episode_size, constraints_dict_1, jerk[:, 0],
                                                 computation_time[:, 0])

    penalty_sum_2, violations_2 = get_violations(n_steps, episode_size, constraints_dict_1, jerk[:, 1],
                                                 computation_time[:, 1])

    return dataset, score, constraints_dict_1, constraints_dict_2, jerk[:, 0], jerk[:, 1], computation_time[:, 0], \
           computation_time[:, 1], penalty_sum_1, penalty_sum_2, violations_1, violations_2


def get_violations(n_steps, episode_size, constraints_dict, jerk, computation_time):
    penalty_sum = 0
    ...
    for i in range(0, n_steps, episode_size):
        for name in constraints_dict.keys():
            if np.any(np.array(constraints_dict[name][i * episode_size: (i + 1) * episode_size]) > 0):
                ...
                violations["Episode " + str(i)].append(name + " violated")

        # TODO set jerk limit to value that makes sense, idk whats good
        if np.any(jerk[i: i + episode_size] > 10000):
            penalty_sum += PENALTY_POINTS["jerk"]
            violations["Episode " + str(i)].append("jerk > 10000")
        ...
        if max_time_violations > 0.2 or mean_time_violations > 0.02:
            ...
            if max_time_violations > 0.2:
                violations["Episode " + str(i)].append("max computation_time > 0.2s")
            else:
                violations["Episode " + str(i)].append("mean computation_time > 0.02s")

        elif max_time_violations > 0.1:
            penalty_sum += PENALTY_POINTS["computation_time_middle"]
            violations["Episode " + str(i)].append("max computation_time > 0.1s")

        elif max_time_violations > 0.02:
            penalty_sum += PENALTY_POINTS["computation_time_minor"]
            violations["Episode " + str(i)].append("max computation_time > 0.02s")
...
if __name__ == "__main__":
    run_remote_tournament("test", "test", "logs", n_episodes=1, n_cores=1, render=False, quiet=False)
